...
const gameOutcome = ['Bob wins!', 'Draw!', 'Alice wins!'];
...
class Shared extends React.Component{
    ...
    async getGuess(){//Fun([], UInt)
        const ghand = await new Promise(resolveHandP =>{
            this.setState({view: 'GetGuess', playable: true, resolveHandP});
        });
        this.setState({view: 'WaitingForResults', ghand});
        return ghand;
    }
    seeOutcome(i) {this.setState({view: 'Done', outcome: gameOutcome[i]})};
    informTimeout() {this.setState({view: 'Timeout'});}
    playHand(hand) {this.state.resolveHandP(hand);}
    guessHand(ghand) {this.state.resolveHandP(ghand);}
}
